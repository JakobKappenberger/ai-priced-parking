...
def train():
    ...
    config_defaults = {
        "lot_distribution_percentage": model_config["training"]["lot_distribution_percentage"],
        ...
        "parking_cars_percentage_increment": model_config["training"]["parking_cars_percentage_intercept"]
    }
    ...
    eval_vec = np.array([0.019966722129783676, 0.0600087958890792, 0.5635815586761667, 0.22410865874363328,
                         0.1665297264211474]).reshape(1, -1)
    train_vec = np.array(
        [n_cars_score, occup_score, np.mean(share_cruising_counter), social_score, speed_score]).reshape(1, -1)
    print(train_vec)
    wandb.log({
    ...
    print(euclidean_distances(eval_vec,
                              train_vec))
